...
class ChromestyleField extends \JFormFieldGroupedList
{
	...
	protected function getGroups()
	{
		...
		foreach ($templateStyles as $template => $styles)
		{
			...
			{
				$tmp = \JHtml::_('select.option', ($template == 'T4' ? '' : $template . '-') . $style, $style);
				$groups[$template][] = $tmp;
			}
		}
	...
	protected function getTemplateModuleStyles()
	{
		...
		// get from chromes layout in T4 BASE
		$chromeLayoutPath = T4PATH_THEMES . '/base/html/layouts/chromes';
