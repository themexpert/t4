...
class Template {
	...
	public function render() {
		...
		if (preg_match_all('/\{t4:([^\}\:]*)\:?([^\}]*)\}/', $layout, $matches)) {
			...
			foreach ($arr as $match) {
				...
				if (method_exists($caller, $func)) $replace[] = $caller->$func();
				else $replace[] = property_exists($doc, $name) ? $doc->$name : $params->get($name, '');
			}
		...
		// render head
		if ($this->isHtml()) $this->renderHead();
		...
		foreach ($themeData->toArray() as $prop => $val) {
			if (preg_match('/_font_family$/', $prop)) {
				...
				if($checkFont !== false){
					...
					if($family) $googleFonts[$family] = empty($googleFonts[$family]) ? $weights : array_merge($googleFonts[$family], $weights);
				} else {
	...
	public function addBodyclass($classes) {
		foreach (explode(' ', $classes) as $class) {
			if (!in_array($class, $this->bodyClass)) $this->bodyClass[] = $class;
		}
	}
	...
	public function beforeRender() {
		...
		if ($this->doc->direction == 'rtl') {
			...
			if ($rtl) $this->doc->addStylesheet($rtl);
		} else {
		...
		if(Path::findInTheme('css/custom.css', true)){
			$this->doc->addStylesheet(Path::findInTheme('css/custom.css', true)."?v=1.1");
		}
	}
	...
	public function renderRow(&$data) {
		...
		if (empty($data['contents']) || !is_array($data['contents'])) return '';
		$cols = $data['contents'];
		...
		foreach ($cols as $col) {
			...
			if (!empty($col['extra_class'])) $cls[] = $col['extra_class'];
	...
	public function renderJdoc($data) {
		...
		//if ($data['jdoc'] == 'modules' && JFactory::getDocument()->countModules($data['name']) == 0) return '';
	...
	public function renderElement($data) {
		...
		if (!empty($data['extra_params']) && preg_match('/^([\w-]*\s*=\s*"[^"]*"\s*)+$/', $data['extra_params'])) {
			...
			if (!preg_match('/title\s*=/', $extra_params)) $extra_params .= ' title="' . md5($extra_params) . '"';
	...
	public function renderBlock($data) {
		...
		if (!$file) $file = Path::findT4Layout('block/' . $data['name'] . '.html');
	...
	public function renderCustomCss() {
		...
		$themecss = Css::renderTheme($rootcss, $rootcss, $params);
		$css .= "\n" . $themecss;
	...
	protected function getLayoutData() {
		...
		if(empty($layout) || empty($layout['sections'])) return null;
		return $layout;
	}


	protected function getThemeData() {
		$data = $this->doc->params->get('theme-settings');
		// custom color
		...
		if (is_string($cc)) {
			...
			foreach ($basecolors as $name => $color) {
				if (empty($customcolors[$name])) $customcolors[$name] = $color;
			}
	...
	protected function getStyleId() {
		...
		if (!empty($template->id)) return $template->id;

		// get template style id from menu
		$active = $app->getMenu()->getActive();
		if ($active && $active->template_style_id != '') return $active->template_style_id;
