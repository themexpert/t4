...
class CustomFont {
    ...
    public static function doAdd() {
        ...
        if (!is_dir(dirname($fontsfile))) \JFolder::create(dirname($fontsfile));
        $output = [
            'ok' => 1,
            'fonts' => $font
        ];
        if(empty($font['type'])) return $output = ['error' => \JText::_('T4_ADDONS_SAVE_ERROR')];
        if($font['type'] == 'css'){
            $fontCss = [];
            foreach ($font['css'] as $css) {
                if(strpos($css,'http') === false){
                    $css_url = \JUri::root() . $css;
                }else{
                    $css_url = $css;
                }
            ...
            foreach ($font['font'] as $fontUrl) {
                if(strpos($fontUrl,'http') === false){
                	...
                	if (!$file) {
                		return $output = ['error' => \JText::_('T4_ADDONS_SAVE_ERROR')];
                	}
